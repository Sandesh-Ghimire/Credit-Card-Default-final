...
if st.button("Predict Risk"):
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["SHAP Explanation", "LIME Explanation", "Model Performance", "Insights"]
    )
    ...
    with tab2:
        ...
        st.write("LIME explains which features influenced this individual prediction.")
        ...
        try:
            ...
            st.write("### Top Local Feature Contributions")

            for feature, weight in exp.as_list():
                icon = "🔴" if weight > 0 else "🟢"
                direction = "increases risk" if weight > 0 else "reduces risk"
                st.write(f"{icon} **{feature}**: {weight:.4f} ({direction})")
